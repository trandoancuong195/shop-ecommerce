...
const SubMenu: React.FC<Props> = ({ subMenuItems }) => {
  ...
  const ArrowDirection = locale !== "fa" ? HiChevronRight : HiChevronLeft;
  ...
  return (
    <div className="flex flex-col px-6 py-5 w-full">
      <div className="flex items-center hover:text-upos-primary transition-color duration-300">
        {subMenuItems ? (
          <>
            <Link
              ...
              return (
                <div className="py-3" key={`${menuTitle}-${index}`}>
                  <Link
                    ...
                    className="block text-sm rtl:ml-10 ltr:mr-10 font-bold px-2 ltr:border-l-4 rtl:border-r-4 border-palette-primary rounded-sm hover:text-upos-primary transition-color duration-300"
                    onClick={() => dispatch(megaMenuActions.closeMegaMenu())}>
                  ...
                  {menuTitle.subtitles.map((subTitle, index) => {
                    return (
                      (<Link
                        ...
                        key={`${subTitle}-${index}`}
                        className="block py-2 hover:text-upos-primary transition-color duration-200"
                        onClick={() =>
